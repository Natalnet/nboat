/*
  ...
  IMU_NEW.cpp - Library for getting curent heading from Compass model HMC6352.
...
// Time to read the sensors again?
  if ((millis() - timestamp) >= OUTPUT_DATA_INTERVAL) {
    ...
    _imuData.eulerAngles.roll = adjustFrame(pitchWFrame);
...
float IMU_NEW::adjustFrame(float angle){
